import { type ChangeEvent, type ComponentPropsWithRef, type KeyboardEvent, useEffect, useId, useRef, useState } from 'react';
...
const Input = ({
  ...
  const variantStyles = {
    ...
    error: 'peer-focus:border-accent-e0 focus:border-accent-e0 border-accent-e0!',
  };

  const finalClassesMix = clsx(errorMessage && variantStyles.error, variantStyles.focus);

  const classNames = {
    container: clsx('relative', fullWidthClass),
    input: clsx(baseInputStyles, finalClassesMix, fullWidthClass, className && className),
    label: clsx(
      baseLabelStyles,
      errorMessage
    ...
    password: clsx(finalClassesMix, `absolute p-1! border-none right-2 top-1/2 transform -translate-y-1/2`),
    extraButton: clsx(
      finalClassesMix,
  ...
  return (
    <div className={classNames.container}>
      <div className={'relative'}>
        {isSearchType && (
          <Button variant="primary" className={classNames.search} disabled={disabled} type="button">
            <Icons.Search />
          </Button>
        )}
        ...
        />
        {label && (
          <Typography htmlFor={finalId} as="label" size="md" className={classNames.label}>
            {label}
          </Typography>
        )}
      ...
      {errorMessage && (
        <Typography color="red" size="sm" weight="normal" as="p" className={classNames.errorMessage}>
          {errorMessage}
        </Typography>
      )}
    </div>
